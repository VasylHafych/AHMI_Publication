...
PATH = "/home/iwsatlas1/vhafych/MPP-Project/AHMI_publication/CaushyDistributionData/caushy_dist_3-iid.jld" # path where data will be saved
...
function run_integrations(dim_range::StepRange{Int64,Int64}, n_repeat::Int64; 
	...
	for dim_run in dim_range
		...
		prior = NamedTupleDist(a = [[Cauchy(0.0, 0.2) for i in 1:dim_run]...],)
		...
		for n_run in 1:n_repeat
			...
			mcmc_ex_time = @CPUelapsed begin 
                samples = bat_sample(prior, nchains*nsamples).result; 
            end
...
dim_range = range(2, step=1, stop=25)
